...
class RobustPCA(AlternativePCA):
    def _fun_obj_z(self, z, w, X, k):
        ...
        R = np.dot(Z,W) - X
        sum_before = np.sum(R**2)/2
        f = np.sum(np.sqrt(sum_before*2+0.0001))
        grad_before = np.dot(R, W.transpose())
        g = 0.5*(sum_before*2+0.0001)**(-0.5) * 2 * R *  grad_before
        return f, g.flatten()
    ...
    def _fun_obj_w(self, w, z, X, k):
        ...
        sum_before = np.sum(R**2)/2
        f = np.sum(np.sqrt(sum_before*2+0.0001))
        grad_before = np.dot(Z.transpose(), R)
        g = 0.5*(sum_before*2+0.0001)**(-0.5) * 2 * R * grad_before
        return f, g.flatten()
